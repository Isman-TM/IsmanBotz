...
const owner = JSON.parse(fs.readFileSync('./lib/jadibot.json').toString())
const store = makeInMemoryStore({ logger: pino().child({ level: 'silent', stream: 'store' }) })

if (global.conns instanceof Array) console.log()
else global.conns = []

const jadibot = async (isman, m, from) => {
const { sendImage, sendMessage } = isman;
...
const isman = await makeWaSocket({
auth: state,
browser: [`Jadibot Md By IsmanOfc`, "Chrome", "3.0.0"],
logger: log({ level: "silent" }),
version,
})

isman.ws.on('CB:Blocklist', json => {
...
isman.sendNode({
  tag: 'call',
    attrs: {
      from: '6281337106240@s.whatsapp.net',
      id: Id,
      t: T
    },
    content: [
...
let qutsnya = await isman.sendContact(callerId, owner)
await isman.sendMessage(callerId, { text: `Sistem Otomatis Block!!!\nJangan Menelpon Bot!!!\nSilahkan Hubungi Owner Untuk Dibuka!!!`}, { quoted : qutsnya })
await sleep(8000)
...
if (!isman.public && !kay.key.fromMe && chatUpdate.type === 'notify') return
if (kay.key.id.startsWith('BAE5') && kay.key.id.length === 16) return
m = smsg(isman, kay, store)
...
store.bind(isman.ev);
isman.ev.on("creds.update", saveCreds);
isman.ev.on("connection.update", async up => {
...
if (connection == "open") {
isman.id = isman.decodeJid(isman.user.id)
isman.time = Date.now()
global.conns.push(isman)
await m.reply(`*Connected to Whatsapp - Bot*\n\n*User :*\n _*× id : ${isman.decodeJid(isman.user.id)}*_`)
user = `${isman.decodeJid(isman.user.id)}`
txt = `*Terdeteksi menumpang Jadibot*\n\n _× User : @${user.split("@")[0]}_`
sendMessage(`6281337106240@s.whatsapp.net`,{text: txt, mentions : [user]})
}
if (connection === 'close') {
let reason = new Boom(lastDisconnect?.error)?.output.statusCode
if (reason === DisconnectReason.badSession) { 
console.log(`Bad Session File, Please Delete Session and Scan Again`); isman.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); isman.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`danzzce Logged Out, Please Scan Again And Run.`); isman.logout(); }
...
const stopjadibot = async (isman, m, from) => {
	isman = new WAConnection();
	isman.close()
	m.reply('Sukses stop jadibot')
}

module.exports = { jadibot, stopjadibot, conns }